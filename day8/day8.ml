...
type const = int
type expr 
type operation = Inc of var * const | Dec of var * const
type comp_opt = Eq | Neq | Ge | Le | Geq | Leq
type condition = Cond of comp_opt * var * const
type instruction = operation * condition
type prog = instruction list
type state = var -> const (* state provides a mapping for vars to consts *)
...
c inc -20 if c == 10
*)
let my_prog: instruction list =
    [
        Inc ("b",5), Cond(Ge, "a", 1);
        Inc ("b",1), Cond(Le, "b", 5);
        Dec ("c",-10), Cond(Geq, "a", 1);
        Dec ("c",-20), Cond(Eq, "c", 10);
    ]
...
let eval_stmt state (stmt: instruction): state = match stmt with
| Inc (_,_), cond
| Dec (_,_), cond -> if not (eval_cond state cond) then state else
    match stmt with
    | Inc(va, c), _ -> (fun v -> if v = va then ((state va) + c) else state v)
    | Dec(va, c), _ -> (fun v -> if v = va then ((state va) - c) else state v)


let execute_prog instructions =
    let state = function
    | _ -> 0
    in
    List.fold_left (fun l -> eval_stmt l) state instructions

let trace_prog (instructions: instruction list) =
    let st = function
    | _ -> 0
    in
    let rec impl (state: state) (instructions: instruction list) (trace: state list) =
        match instructions with
        | [] -> trace
        | h::t -> let s1 = eval_stmt state h in
        impl s1 t (state :: trace)
    in
    impl st instructions []

let get_assigned_registers prog = List.map (fun instr ->
    let (op, cond) = instr in
    match op with
    | Inc(v,_) | Dec(v,_) -> v
) prog

let get_state_vals state vars = List.map (fun v -> state v) vars

let list_max l = List.fold_left (fun acc el -> if el > acc then el else acc) (List.nth l 0) l

let get_final_state prog =
    let final_state = execute_prog prog in
    let assigned_vars = get_assigned_registers prog in
    List.map(fun var -> final_state var) assigned_vars

let solution prog = get_final_state prog |> list_max

let solution_two prog =
print_int (List.length (trace_prog prog));
let vars = get_assigned_registers prog in 
trace_prog prog |> List.map (fun v -> 
    List.map(fun var -> 
        v var
    ) vars
) |> List.flatten
...
|> List.map (fun tokens ->
    match tokens with
    | [va; op; const; _; creg; cop; cconst] ->
        let const = int_of_string const in
        let cconst = int_of_string cconst in
        let cop = begin match cop with
        | "<" -> Le
        | ">" -> Ge
        | "<=" -> Leq
        | ">=" -> Geq
        ...
        let operation = begin match op with 
        | "inc" -> Inc (va, const)
        | "dec" -> Dec (va, const)
        | _ -> failwith "damn"
        end in
        let condition = Cond (cop, creg, cconst) in
        operation, condition
...
let in_str = "yxr inc 119 if nev != 6
piw dec -346 if tl != 4
