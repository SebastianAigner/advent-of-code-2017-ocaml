type yell = Yell of string * (string list)
...
[
    ...
    Yell("cntj", []);
    Yell("ktlj", []);
    Yell("fwft", ["ktlj"; "cntj"; "xhth"]);
]

let get_left_sides yell_list = List.map
    (fun yell -> match yell with
        | Yell(x,_) -> x
    )
    yell_list

let get_right_sides yell_list = List.map
    (fun yell -> match yell with
        | Yell(_,y) -> y
    )
    yell_list
...
let get_root yell_list =
    let left = get_left_sides yell_list in
    let right = get_right_sides yell_list in
    (* we need those elements of the left side that do not appear in the right side *)
    List.filter (fun le -> not (List.mem le right)) left

let rec sanitized = function
| [] -> []
| h::t -> (if h <> "" then [h] else []) @ sanitized t

let parse_input string =
    let lines = String.split_on_char '\n' string |> sanitized in
    let lines_with_right_side =
    lines |> List.filter (fun line -> String.contains line '>') in
    let lines_with_no_right_side =
    lines |> List.filter (fun line -> not ( String.contains line '>')) in
    let lonly = lines_with_no_right_side
        |> List.map
            (fun line ->
                let name = List.nth (String.split_on_char ' ' line) 0 in
                Yell(name, [])
            )
    in
    let both = lines_with_right_side
        |> List.map
            (fun line ->
                let name = List.nth (String.split_on_char ' ' line) 0 in
                let ptrs = List.nth (String.split_on_char '>' line) 1
                |> String.split_on_char ','
                |> List.map (fun str -> String.trim str) in
                Yell(name, ptrs)
            )
    in
    lonly @ both;;
