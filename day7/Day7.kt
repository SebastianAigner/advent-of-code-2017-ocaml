class Node(var weight: Int, var children: List<String>, var tree: Map<String, Node> ) {
    fun isBalanced(): Boolean {
        val children = tree.entries.filter { children.contains(it.key) }
        //println("I have children $children")
        val childWeights = children.map { it.value.weight }
        //println("$childWeights")
        if (childWeights.isEmpty()) {
            return true
        }
        val balanced = childWeights.distinct().size == 1
        val childNodes = children.map { it.value }
        if(!balanced) {
            if(childNodes.all { it.children.isEmpty() }) {
                println("Hey look! $childWeights")
            }
        }
        return balanced
    }
...
fun main(args: Array<String>) {
    ...
    actualInString.trim().split("\n").forEach {
        val fields = it.split(" ")
        //fwft|(72)|->|ktlj,|cntj,|xhth
        val name = fields[0]
        val weight = fields[1].drop(1).dropLast(1).toInt()
        val refs =
                if (fields.size == 2) { emptyList() }
                else fields.subList(3, fields.lastIndex).map { it.filter { it != ',' } }
        nodes[name] = Node(weight, refs, nodes)
    }
    println(nodes)
    nodes.forEach { (k,v) ->
        v.isBalanced()
